...
> ✨ **New:** any widget can now accept **files dropped from the OS file manager** — just call `widget.accept_dropped_files(callback)` (see [Drag and drop](#drag-and-drop)). There is also a new embedded 3D mesh viewer, [`View3D`](#classes) and a SVGImage.
