...
class SpeckleApp(App):
    def __init__(self):
        ...
        self.contrast.grid_into(
            self.controls, row=0, column=0, padx=10, pady=10, sticky="nw"
        )

        self.grid_label = Label("Grid size:")
        self.grid_label.grid_into(
            self.controls, row=0, column=2, padx=10, pady=10, sticky="ne"
        )
        self.grid_count_entry = NumericEntry(value=5, width=3, minimum=1)
        self.grid_count_entry.grid_into(
            self.controls, row=0, column=3, padx=10, pady=10, sticky="nw"
        )
        self.grid_count_entry.value_variable.trace_add("write", self.grid_count_updated)
        self.show_grid_checkbox = Checkbox("Show grid")
        self.show_grid_checkbox.grid_into(
            self.controls, row=0, column=4, padx=10, pady=10, sticky="nw"
        )
        self.rescalable_checkbox = Checkbox("Rescalable")
        self.rescalable_checkbox.grid_into(
            self.controls, row=0, column=5, padx=10, pady=10, sticky="nw"
        )
        ...
        self.filemanager.grid_into(
            self.window, row=0, column=0, rowspan=3, sticky="nsew"
        )
        self.filemanager.row_resize_weight(0, 0)
        self.filemanager.row_resize_weight(1, 1)
        self.button_load = Button(
            "Select directory…", user_event_callback=self.click_load
        )
        self.button_load.grid_into(
            self.filemanager, row=0, column=0, padx=10, pady=10, sticky="nw"
        )
        self.regex_entry = LabelledEntry("Sort on regex:", text=r"._(\d+.\d+)\.")
        self.regex_entry.grid_into(
            self.filemanager, row=0, column=1, padx=10, pady=10, sticky="nw"
        )
        ...
        self.filesview = TableView(
            columns={
                "filename": "Filename",
                "contrast": "Contrast",
                "mean_tiled_contrast": "{0}x{0}".format(grid_count),
            }
        )
        self.filesview.delegate = self
        self.filesview.grid_into(
            self.filemanager,
            row=1,
            column=0,
            columnspan=4,
        ...
        self.plot.grid_into(
            self.window, row=4, column=0, columnspan=4, padx=20, pady=20, sticky="nsew"
        )
        ...
        self.image.bind_property_to_widget_value(
            "is_grid_showing", self.show_grid_checkbox
        )
        self.image.bind_property_to_widget_value(
            "is_rescalable", self.rescalable_checkbox
        )
        ...
        while True:
            ...
            try:
                ...
                if item_id is not None:
                    ...
                    contrast = std / mean
                    contrasts_NxN = image_contrasts(
                        pil_array, M=grid_count, N=grid_count
                    )
                    contrast_mean_NxN = np.mean(contrasts_NxN)

                    pil_image.close()
                    self.results_queue.appendleft(
                        (item_id, contrast, contrast_mean_NxN, contrasts_NxN)
                    )
    ...
    def get_calculations_from_queue(self, event):
        try:
            while True:
                ...
                self.filesview.widget.set(
                    item_id, column=1, value="{0:.3f}".format(contrast)
                )
                self.filesview.widget.set(
                    item_id, column=2, value="{0:.3f}".format(contrast_mean_NxN)
                )
                ...
                for i, contrast in enumerate(contrasts_NxN):
                    self.filesview.widget.insert(
                        item_id,
                        END,
                        values=("  Grid element #{0}".format(i), "", contrast, None),
                    )
    ...
    def update_plot(self, event, object):
        ...
        self.plot.first_axis.set_ylabel(
            "Contrast sigma/mean for {0}x{0}".format(grid_count)
        )
    ...
    def refresh_tile_contrasts_table(self, contrasts):
        ...
        self.contrast.value_variable.set(
            "Contrast: {0:.3f} ± {1:.3f} (±{2:.1f}%)".format(
                contrast_mean, contrast_std, contrast_err
            )
        )
